...
func buildSCCRQ() ([]byte, []byte, error) {
	...
	_ = binary.Write(&packet, binary.BigEndian, flags)

	// Length will be calculated and written later
	lengthPos := packet.Len()
	_ = binary.Write(&packet, binary.BigEndian, uint16(0)) // Placeholder for length

	// Tunnel ID (recipient, set to 0 for new connection)
	_ = binary.Write(&packet, binary.BigEndian, uint16(0))
...
func buildAVP(avpType uint16, value []byte, mandatory bool) []byte {
	...
	_ = binary.Write(&avp, binary.BigEndian, flags)

	// Vendor ID (2 bytes) - 0 for IETF
	_ = binary.Write(&avp, binary.BigEndian, uint16(0))

	// Attribute Type (2 bytes)
	_ = binary.Write(&avp, binary.BigEndian, avpType)
