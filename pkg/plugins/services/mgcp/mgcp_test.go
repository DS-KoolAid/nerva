...
func TestPlugin_Run_SuccessWithOK(t *testing.T) {
	...
	go func() {
		buf := make([]byte, 4096)
		_, _ = server.Read(buf) // consume probe
		_, _ = server.Write([]byte("200 9 OK\r\n\r\n"))
		server.Close()
	}()
...
func TestPlugin_Run_CiscoIOS(t *testing.T) {
	...
	go func() {
		buf := make([]byte, 4096)
		_, _ = server.Read(buf)
		_, _ = server.Write([]byte(ciscoResponse))
		server.Close()
	}()
...
func TestPlugin_Run_WrongTransactionID(t *testing.T) {
	...
	go func() {
		buf := make([]byte, 4096)
		_, _ = server.Read(buf)
		_, _ = server.Write([]byte("200 42 OK\r\n\r\n"))
		server.Close()
	}()
...
func TestPlugin_Run_MEGACOResponse(t *testing.T) {
	...
	go func() {
		buf := make([]byte, 4096)
		_, _ = server.Read(buf)
		_, _ = server.Write([]byte("MEGACO/1 [10.0.0.1]\nReply = 1 {}\n"))
		server.Close()
	}()
...
func TestPlugin_Run_DNSResponse(t *testing.T) {
	...
	go func() {
		buf := make([]byte, 4096)
		_, _ = server.Read(buf)
		_, _ = server.Write([]byte{0x00, 0x01, 0x81, 0x80, 0x00, 0x01, 0x00, 0x01})
		server.Close()
	}()
...
func TestPlugin_Run_ResponseWithMGCPCommand(t *testing.T) {
	...
	go func() {
		buf := make([]byte, 4096)
		_, _ = server.Read(buf)
		_, _ = server.Write([]byte("RSIP 39380951 * MGCP 1.0\r\nRM: restart\r\n\r\n"))
		server.Close()
	}()
...
func TestPlugin_Run_ManyEndpoints(t *testing.T) {
	...
	go func() {
		buf := make([]byte, 4096)
		_, _ = server.Read(buf)
		_, _ = server.Write([]byte(response.String()))
		server.Close()
	}()
